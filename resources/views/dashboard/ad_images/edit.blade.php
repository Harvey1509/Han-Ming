    ...
    <div class="main-content__form-wrapper">
        ...
        <form action="{{ route('ad_images.update', $imagen->id) }}" class="form-content form-imagenes" method="POST"
            ...
            <div class="form-column form-imagenes-column inputs">
                <x-input type="text" label="ID" value="{{ $imagen->id }}" status="disabled" />
                <x-input name="fecha_inicio" type="date" label="Fecha de Inicio" value="{{ $imagen->fecha_inicio }}" />
                <x-input name="fecha_fin" type="date" label="Fecha de Fin" value="{{ $imagen->fecha_fin }}" />
                <x-input name="tipo" type="text" label="Tipo" value="{{ $imagen->tipo }}" />
                <x-input name="orden" type="text" label="Orden" value="{{ $imagen->orden }}" />
                <x-select name="estado" label="Estado" text="Seleccione un estado" :options="['Activo' => 'Activo', 'Inactivo' => 'Inactivo']" :currentOption="$imagen->estado" />
            </div>
            <div class="form-column form-imagenes-column image">
                <x-file-upload name="url_imagen" label="Cambiar Imagen" :src="$imagen->url_imagen" />
            </div>
        </form>
